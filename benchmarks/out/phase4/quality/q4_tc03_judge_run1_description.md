# tc03_judge — q4  run 1/3  (judgment)
...
1.  Route 53 DNS / CloudFront CDN (最も上部にあるサービス群)
2.  ALB / NAT GW (パブリックサブネット内のロードバランサーとNATゲートウェイ)
3.  ECS Fargate App Service / ElasticCache Redis (プライベートサブネット内のアプリケーション層およびキャッシュ層)
4.  RDS PostgreSQL Primary / RDS PostgreSQL Read Replica (データストア層)
...
*   **緑色で太い線:**
    ...
    *   （意味の違いについて図から読み取れる情報はありませんが、接続されているコンポーネント間の通信を示しています。）

*   **青緑/シアンで太い線:**
    *   ALB (AZ-a) $\rightarrow$ ECS Fargate App Service (AZ-a)
    *   ALB (AZ-b) $\rightarrow$ ECS Fargate App Service (AZ-b)
    *   （意味の違いについて図から読み取れる情報はありませんが、ロードバランサーからアプリケーション層へのトラフィックフローを示しています。）
