...
**何をしたか:** LM Studio 上で Gemma 4 E4B の Q4_K_M / Q5_K_M / Q8_0 をそれぞれロード、S2 / S3 速度実測 + `tc01..tc04` で品質を Gemini 2.5 Flash judge で採点。

**結論: 第一候補 Q4_K_M、backup Q5_K_M**

- 速度はファイルサイズに比例（CPU は memory-bandwidth bound）
- 品質は 3 量子化とも 0.72〜0.76 で統計的に区別不能 — Q4 がたまたま最高値
- Q4 が「品質トップタイ AND 最速 AND メモリ最小」で完全優勢、Q8 を排除

**成果物:** [03-model-selection.md](03-model-selection.md) + `benchmarks/out/phase4/` + 評価スクリプト [`tests/text_vs_image/phase4_quality_eval.py`](../../../tests/text_vs_image/phase4_quality_eval.py)
...
**何をするか（未着手）:** target mini PC（i5-14500T、32 GB RAM、iGPU only）に LM Studio + Gemma 4 E4B Q4_K_M をインストールし、S2 / S3 を 1 回ずつ走らせて、dev rig CPU-only モードの値と比較する。

**判定基準:**

- 差が **2x 以内** → 「使用可能」
- 差が 2x を超えるが完走 → 「限界的」
- 完走しない（timeout / OOM）→ 「不可」 → backup の Q5_K_M で再判定 or E2B など他バリアントへの切替検討
...
- 採用ホスト（現状: LM Studio）
- 第一候補量子化（現状: Gemma 4 E4B Q4_K_M）
- backup 量子化（現状: Gemma 4 E4B Q5_K_M）
...
- ✅ **Phase 1〜5 完了** — 採用候補（LM Studio + Gemma 4 E4B Q4_K_M）まで決定済み、プロトタイプ動作確認済み
