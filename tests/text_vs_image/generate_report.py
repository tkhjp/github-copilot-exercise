...
<head>
<meta charset="utf-8" />
<title>画像の文字化 vs 画像直接入力 — 精度比較レポート v2</title>
...
#sidenav .nav-group { margin-bottom: 10px; }
#sidenav .nav-heading {
  font-weight: 700;
  font-size: 11px;
...
#sidenav .nav-badge.p1 { background: var(--accent); }
#sidenav .nav-badge.p2 { background: #16a34a; }
#sidenav a {
  color: var(--muted);
...
.mermaid-container { text-align: center; margin: 24px 0; }
.fact-table tbody tr:hover { background: #f0f9ff; }
...
<!-- ===== SIDE NAVIGATION ===== -->
<nav id="sidenav">
  <div class="nav-title">精度比較レポート v2</div>
  ...
  <div class="nav-group">
    <a href="#future" style="padding-left:8px;">今後の課題</a>
  </div>
</nav>

<!-- ===================== HEADER ===================== -->
<h1>画像の文字化 vs 画像直接入力 — 精度比較レポート v2</h1>
<p class="subtitle">
  <strong>Phase 1</strong> = Gemini モデル比較（どのモデルがより良い画像記述を生成するか）<br>
  <strong>Phase 2</strong> = GPT-5.4 テキスト vs 画像（テキスト記述と画像直接入力のどちらが高精度か？）
</p>

<table class="model-table">
  <tr><th>役割</th><th>モデル</th><th>用途</th></tr>
  <tr><td>Describe</td><td>gemini-3-flash / gemini-3.1-flash-lite</td><td>画像からテキスト記述を生成（Phase 1）</td></tr>
  <tr><td>Answer</td><td>GPT-5.4</td><td>テキストまたは画像を使って質問に回答（Phase 2）</td></tr>
  <tr><td>Judge</td><td>GPT-5.4</td><td>正解ファクトに対して回答を採点</td></tr>
</table>

<div class="legend">
  <span class="score-present">&#10003;</span> 正確に含まれている (1.0点)
  <span class="score-partial">&#9651;</span> 部分的に含まれている (0.5点)
  <span class="score-missing">&#10007;</span> 含まれていない (0.0点)
  <span class="score-na">&mdash;</span> 未採点
</div>
...
<div class="section" id="phase1">
  <h2>Phase 1: 記述モデル比較</h2>
  <p class="muted">2つの Gemini モデルがそれぞれ2種類の記述（汎用プロンプト + 専用プロンプト）を画像ごとに生成。正解ファクトと照合して採点。情報抽出ケースのみ対象。</p>

  <div class="mermaid-container" id="mermaid-flow">
    <div class="mermaid">
flowchart LR
    ...
    <p class="muted" style="margin-top:8px;">D1-D4 = 各モデル&times;各プロンプトで生成した画像記述。A1 = テキスト記述経由で回答、A2 = 画像直接入力で回答。</p>
  </div>
  ...
  <!-- Describe matrix -->
  <h3>記述マトリックス概要 (情報抽出ケース)</h3>
  <div id="matrix-section"></div>
</div>

<!-- ===================== PHASE 1 DETAIL ===================== -->
<div id="phase1-detail">
  <h2>Phase 1: ケース別詳細 (情報抽出)</h2>
  <div id="phase1-cases"></div>
</div>

<!-- ===================== PHASE 2 ===================== -->
<div class="section" id="phase2">
  <h2>Phase 2: 回答精度の比較 (情報抽出テスト)</h2>
  <p class="muted">GPT-5.4 が2つの経路で質問に回答: A1 = テキスト記述経由（Phase 1 採用モデルの汎用+専用記述）、A2 = 画像直接入力。</p>

  <div class="flow-diagram">A1 (テキスト経由): D_汎用 + D_専用 + 質問  -->  GPT-5.4 テキスト --> 回答
A2 (画像直接):              画像 + 質問  -->  GPT-5.4 ビジョン --> 回答</div>
...
<div id="phase2-judgment">
  <h2>Phase 2: 回答精度の比較 (判断テスト)</h2>
  <p class="muted">判断ケースは事実抽出ではなく推論の質をテスト。推論ポイントに対して採点。</p>
...
<div class="section" id="future">
  ...
  <ul class="future-list">
    <li><span class="future-tag">Pipeline</span> pptx 画像抽出パイプライン &mdash; スライド&rarr;画像変換の自動化</li>
    <li><span class="future-tag">Quality</span> 画像の前処理 &mdash; コントラスト補正、ノイズ除去、解像度正規化</li>
    <li><span class="future-tag">Routing</span> 画像種別の自動判定 &mdash; 画像内容に基づく専用プロンプトの自動選択</li>
    <li><span class="future-tag">Integration</span> Copilot Chat 統合 &mdash; describe-then-answer パイプラインの GitHub Copilot への組み込み</li>
  </ul>
</div>
...
function renderMatrix() {
  ...
  let html = '<table class="matrix-table"><thead><tr><th></th><th>汎用</th><th>専用</th><th>平均</th></tr></thead><tbody>';
  ...
  if (p1.selected_model) {
    html += `<div class="phase1-banner">
      <strong>採用モデル: ${esc(p1.selected_model)}</strong><br>
...
function renderPhase1Cases() {
  ...
  for (const c of extCases) {
    ...
    let factHtml = '<table class="fact-table"><thead><tr><th>#</th><th>正解ファクト</th>';
    factHtml += '<th style="width:60px;text-align:center;background:var(--flash-bg);">D1 (Flash/汎用)</th>';
    factHtml += '<th style="width:60px;text-align:center;background:var(--flash-bg);">D2 (Flash/専用)</th>';
    factHtml += '<th style="width:60px;text-align:center;background:var(--lite-bg);">D3 (Lite/汎用)</th>';
    factHtml += '<th style="width:60px;text-align:center;background:var(--lite-bg);">D4 (Lite/専用)</th>';
    ...
    for (const label of ['D1','D2','D3','D4']) {
      ...
      descCards += `
        <div class="desc-card ${col.tint}">
          <button class="expand-btn" onclick="openModal('${c.id}','describe','${label}')">&#9974; 全文表示</button>
    ...
    div.innerHTML = `
      ...
      <div class="case-body">
        ...
        <div>
          ...
          <h3>正解ファクト vs 記述の採点結果</h3>
          ${factHtml}
        </div>
      </div>
      <h3>LLM が生成した記述テキスト
        <span class="muted" style="font-size:12px;">&mdash;
          <span style="color:#1d4ed8;">青 = Flash (D1/D2)</span>、
          <span style="color:#7c3aed;">紫 = Lite (D3/D4)</span>
        </span>
      </h3>
...
function renderPhase2() {
  ...
  let sumHtml = '<table class="cmp-table"><thead><tr><th>ケース</th><th>A1 (テキスト経由)</th><th>A2 (画像直接)</th><th>参考: 記述スコア</th></tr></thead><tbody>';
  for (const c of extCases) {
  ...
  sumHtml += `<td style="text-align:left;">平均</td>`;
  ...
  for (const c of extCases) {
    ...
    let factHtml = '<table class="fact-table"><thead><tr><th>#</th><th>正解ファクト</th>';
    factHtml += '<th style="width:80px;text-align:center;background:var(--answer-bg);">A1 (テキスト経由)</th>';
    factHtml += '<th style="width:80px;text-align:center;background:var(--answer-bg);">A2 (画像直接)</th>';
    factHtml += '</tr></thead><tbody>';
    ...
    for (const label of ['A1','A2']) {
      const col = c.answer_cols[label];
      const pctStr = col.pct !== null ? col.pct.toFixed(1)+'%' : '\u2014';
      const fullLabel = label === 'A1' ? 'A1: テキスト経由' : 'A2: 画像直接';
      answerCards += `
        <div class="answer-card">
          <button class="expand-btn" onclick="openModal('${c.id}','answer','${label}')">&#9974; 全文表示</button>
    ...
    div.innerHTML = `
      ...
      <h3>ファクト採点: テキスト経由 vs 画像直接</h3>
      ${factHtml}
      <h3>GPT-5.4 の回答テキスト</h3>
...
function renderJudgmentCases() {
  ...
  for (const c of judgCases) {
    ...
    // Fact table with A1/A2
    let factHtml = '<table class="fact-table"><thead><tr><th>#</th><th>推論ポイント</th>';
    factHtml += '<th style="width:80px;text-align:center;background:var(--answer-bg);">A1 (テキスト経由)</th>';
    factHtml += '<th style="width:80px;text-align:center;background:var(--answer-bg);">A2 (画像直接)</th>';
    factHtml += '</tr></thead><tbody>';
...
// =========================================================================
// =========================================================================
// Side navigation: populate case links + scroll spy
// =========================================================================
function buildSideNav() {
  const extCases = DATA.cases.filter(c => c.test_type === 'extraction');
  const jdgCases = DATA.cases.filter(c => c.test_type === 'judgment');

  // Phase 1 cases
  const p1el = document.getElementById('nav-p1-cases');
  if (p1el) {
    let h = '';
    for (const c of extCases) {
      const short = c.title.split('(')[0].replace(/\s+/g, ' ').trim();
      h += `<a href="#p1-${c.id}">${c.id} ${short}</a>`;
    }
    p1el.innerHTML = h;
  }
  // Phase 2 extraction cases
  const p2ext = document.getElementById('nav-p2-ext-cases');
  if (p2ext) {
    let h = '';
    for (const c of extCases) {
      const short = c.title.split('(')[0].replace(/\s+/g, ' ').trim();
      h += `<a href="#p2-${c.id}">${c.id} ${short}</a>`;
    }
    p2ext.innerHTML = h;
  }
  // Phase 2 judgment cases
  const p2jdg = document.getElementById('nav-p2-jdg-cases');
  if (p2jdg) {
    let h = '';
    for (const c of jdgCases) {
      const short = c.title.replace('[JUDGMENT] ', '').split('(')[0].replace(/\s+/g, ' ').trim();
      h += `<a href="#j-${c.id}">${c.id} ${short}</a>`;
    }
    p2jdg.innerHTML = h;
  }
}
