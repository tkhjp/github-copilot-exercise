...
_生成日: 2026-04-26 (extraction_report.py で自動生成、`python tests/text_vs_image/extraction/extraction_report.py` で再生成可)_
...
- PNG recall: **-0.171** (0.894 → 0.723)
- PPTX recall: **-0.013** (0.901 → 0.887)
...
### 9.3 v1 prompt × v2 corpus (ceiling fix の効果検証)

**同じ v1 prompt** をそのまま v2 corpus に投入。同条件 (prompt 一定) で corpus 難化が
どれだけ recall を下げるかを観測した。結果は §3 / §4 / §5 のテーブルを参照。
